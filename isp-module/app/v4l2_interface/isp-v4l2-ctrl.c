...
int isp_v4l2_ctrl_init( isp_v4l2_ctrl_t *ctrl )
{
    ...
    /* 4.9->6.1 port: v4l2_ctrl_add_handler() gained a trailing
     * `bool from_other_dev` param. hdl_std_ctrl and hdl_cst_ctrl are both
     * this driver's own two control handlers (see isp_v4l2_ctrl_t),
     * not a handler borrowed from a different v4l2_device, so false. */
