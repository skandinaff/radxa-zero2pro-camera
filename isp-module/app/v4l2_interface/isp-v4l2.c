...
#include <linux/dma-mapping.h> /* dma_alloc_coherent()/dma_free_coherent(),
                                 * used by isp_cma_alloc()/isp_cma_free()
                                 * below -- see the port comment there. */
...
static int isp_v4l2_fh_open( struct file *file )
{
    ...
    if ( stream_opened > V4L2_STREAM_TYPE_MAX ) {
        ...
        /* 4.9->6.1 port: kzfree() -> kfree_sensitive(), same rename as in
         * isp-v4l2-stream.c's isp_v4l2_stream_deinit(). */
        kfree_sensitive( sp );
        return -EBUSY;
    }
...
static int isp_v4l2_qbuf( struct file *file, void *priv, struct v4l2_buffer *p )
{
    ...
    /* 4.9->6.1 port: vb2_qbuf() gained a `struct media_device *mdev` param
     * (used to update Media Controller request-API pipeline links on
     * qbuf). This driver never registers a media_device anywhere (grepped
     * the whole tree -- no `.mdev` on v4l2_dev, no media_device_init()
     * call), so NULL is the correct value here, not a placeholder guess. */
...
/* 4.9->6.1 port: struct v4l2_ioctl_ops.vidioc_cropcap/.vidioc_g_crop/
 * .vidioc_s_crop were removed entirely -- the core switched to
 * implementing the (still-supported, still-valid-UAPI) VIDIOC_CROPCAP/
 * VIDIOC_G_CROP/VIDIOC_S_CROP ioctls generically on top of
 * .vidioc_g_selection/.vidioc_s_selection (see v4l2-ioctl.c's
 * v4l2_g_crop()/v4l2_s_crop()/v4l2_cropcap() shims, which translate to
 * the selection API and call through to these ops). So userspace apps
 * that still call the legacy cropcap/g_crop/s_crop ioctls keep working
 * unchanged; only this driver's internal registration point moved.
 * struct v4l2_crop/v4l2_cropcap themselves are untouched UAPI types
 * (still valid, still used below and in isp-v4l2-stream.c's
 * isp_v4l2_get_cropcap()/isp_v4l2_get_crop()/isp_v4l2_set_crop(), which
 * needed no changes), only the driver-facing ioctl_ops entry points did.
 *
 * V4L2_SEL_TGT_CROP        -> current crop rectangle (was G/S_CROP)
 * V4L2_SEL_TGT_CROP_DEFAULT -> cropcap.defrect
 * V4L2_SEL_TGT_CROP_BOUNDS  -> cropcap.bounds
 * cropcap.pixelaspect has its own separate replacement op,
 * .vidioc_g_pixelaspect, wired up below too so that information isn't
 * silently dropped. */
                                ...
                                struct v4l2_selection *s)
{
    ...
    case V4L2_SEL_TGT_CROP_BOUNDS:
        ...
        s->r = (s->target == V4L2_SEL_TGT_CROP_DEFAULT) ? cap.defrect : cap.bounds;
        break;
    default:
        return -EINVAL;
    }
...
static const struct v4l2_ioctl_ops isp_v4l2_ioctl_ops = {
    ...
    /* 4.9->6.1 port: there is no *_mplane variant of vidioc_enum_fmt_*;
     * VIDIOC_ENUM_FMT doesn't distinguish single- vs multi-planar (the
     * format is identified by fmtdesc->type, same callback serves both),
     * so this field was never split the way g/s/try_fmt were. Checked
     * v4l2-ioctl.h -- only "vidioc_enum_fmt_vid_cap" exists. */
    ...
    /* crop ioctls -- selection API, see the port comment above
     * isp_v4l2_g_selection() for why these replaced
...
/* 4.9->6.1 port, REVISED after real modpost testing: same story as
 * cma_alloc()/cma_free() in isp-vb2-cmalloc.c -- dma_alloc_from_contiguous()/
 * dma_release_from_contiguous() are not EXPORT_SYMBOL'd on this kernel.
 * Confirmed for real:
 *   ERROR: modpost: "dma_release_from_contiguous" [...] undefined!
 *   ERROR: modpost: "dma_alloc_from_contiguous" [...] undefined!
 * Switched to dma_alloc_coherent()/dma_free_coherent(), the only exported
 * path to CMA-backed memory from an out-of-tree module. Same NOT-VERIFIED
 * linear-map caveat applies here as documented at length in
 * isp-vb2-cmalloc.c's cma_alloc() -- see that comment, it's the same
 * risk for the same reason (isp_kaddr below feeds virt_to_phys() callers
 * elsewhere in this driver the same way vb2_cmalloc_buf.vaddr does). */
static dma_addr_t isp_cma_dma_handle;

static int isp_cma_alloc(struct platform_device *pdev, unsigned long size)
{
    isp_kaddr = dma_alloc_coherent(&pdev->dev, size, &isp_cma_dma_handle, GFP_KERNEL);
    if (!isp_kaddr) {
        LOG(LOG_ERR, "Failed alloc cma pages.\n");
        return -1;
    }
    isp_paddr = isp_cma_dma_handle;
...
static void isp_cma_free(struct platform_device *pdev, void *kaddr, unsigned long size)
{
    ...
    dma_free_coherent(&pdev->dev, size, kaddr, isp_cma_dma_handle);
}
...
int isp_v4l2_create_instance( struct v4l2_device *v4l2_dev, struct platform_device *pdev)
{
    ...
    /* 4.9->6.1 port: VFL_TYPE_GRABBER renamed to VFL_TYPE_VIDEO (plain
