...
#include <linux/version.h> /* LINUX_VERSION_CODE/KERNEL_VERSION were pulled in
                             * transitively on 4.9 via some other header; not
                             * so on 6.1, so the version-gated blocks below
                             * need it explicitly. Target is always >= 4.4.0
                             * so left the gates in place rather than pruning
                             * the dead branches -- keeps the diff smaller. */
