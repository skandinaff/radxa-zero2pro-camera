...
#include <linux/ktime.h> /* ktime_get()/ktime_sub()/ktime_to_ms(), replacing
                           * struct timeval/do_gettimeofday() -- see the
                           * isp_v4l2_stream_fill_buf() port comment. */
...
/* 4.9->6.1 port: <linux/dma-contiguous.h> is gone; dropped rather than
 * redirected to dma-map-ops.h since nothing in this file actually calls
 * anything from it (dma_alloc_from_contiguous et al are used in
 * isp-vb2-cmalloc.c, not here). */
...
void isp_v4l2_stream_deinit( isp_v4l2_stream_t *pstream )
{
    ...
    /* release fw_info */
    if ( pstream ) {
        /* 4.9->6.1 port: kzfree() (a plain rename shim over
         * kfree_sensitive()) was removed once the rename fully landed;
         * kfree_sensitive() is the direct replacement (zeroes the
         * memory before freeing, same as kzfree always did). */
...
void isp_v4l2_stream_fill_buf( isp_v4l2_stream_t *pstream, isp_v4l2_buffer_t *buf, uint32_t *hw_buf_offset )
{
    ...
    /* 4.9->6.1 port: struct timeval/do_gettimeofday() were removed as
     * part of the y2038 cleanup (struct timeval's 32-bit tv_sec overflows
     * in 2038; the whole API was deleted, not just deprecated). Only
     * ever used here for a millisecond duration measurement (see
     * interval_val below), so ktime_t + ktime_get() is the direct,
     * simpler modern replacement -- no calendar time is actually needed. */
    ktime_t begin, end;
    ...
    interval_val = ktime_to_ms( ktime_sub( end, begin ) );
