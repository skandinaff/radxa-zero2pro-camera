...
#include <linux/clk.h>
#include <linux/property.h> /* fwnode_graph_get_next_endpoint() etc, for the
                              * v4l2_async notifier rewrite -- see the
                              * V4L2_ASYNC_MATCH_CUSTOM comment below. */
...
struct acamera_v4l2_subdev_t {
    ...
    /* 4.9->6.1 port: the fixed-size soc_async_sd[]/soc_async_sd_ptr[]
     * arrays are gone -- see the notifier.ops / v4l2_async_nf_* rewrite
     * below (near isp_platform_probe) for why, and PORT_NOTES.md for the
     * full explanation. The new v4l2_async core owns async-subdev
     * lifetime itself via notifier->asd_list once you add entries with
     * v4l2_async_nf_add_fwnode()/friends, so there's nothing left for
     * this struct to track except the bound subdevs themselves. */
    int subdev_counter;
...
static const struct v4l2_async_notifier_operations acamera_notifier_ops = {
...
int  write_to_file (char *fname, char *buf, int size)
{
    ...
    /* 4.9->6.1 port: get_fs()/set_fs(KERNEL_DS) was removed (the whole
     * set_fs address-limit override mechanism is gone on modern kernels).
     * kernel_write() is the direct replacement for writing from a kernel
     * buffer without needing to fake a user address space. */
    ...
    /* open file to write */
        ...
        /* 4.9->6.1 port: filp_open() has always returned ERR_PTR() on
         * failure, never NULL -- the original "if (!fp)" check could
         * never trigger and a failed open would fall through to
         * vfs_write() on an error pointer. Fixed to IS_ERR() while
         * porting this function anyway. */
        if (IS_ERR(fp)) {
...
static void hw_reset(bool reset)
{
    ...
    reset_addr = ioremap(0xffd01090, 8);;//ioremap(0xffd01014, 8);
    if (reset_addr == NULL) {
...
static int32_t isp_platform_probe( struct platform_device *pdev )
{
    ...
    /* 4.9->6.1 port: V4L2_ASYNC_MATCH_CUSTOM (an arbitrary-match-function
     * escape hatch the original code used with match=NULL to mean "bind
     * whatever async subdev shows up, no filtering") was removed from
     * the v4l2_async core well before 6.1 -- mainline only supports
     * V4L2_ASYNC_MATCH_I2C and V4L2_ASYNC_MATCH_FWNODE now, both of which
     * require actually knowing what to match against. There's no
     * "match anything" option left by design.
     *
     * Replaced with the standard modern pattern: walk this device's own
     * OF-graph endpoints (port/endpoint nodes under the isp@ff140000 DT
     * node) and fwnode-match whatever's on the other end of each one,
     * same idea as e.g. rkisp1's notifier setup. This is the mainline-
     * correct way to do it, but it depends on an endpoint actually
     * existing in the devicetree linking the ISP to the sensor (imx415).
     * That endpoint does NOT exist yet as of this port (out of scope for
     * this pass -- see PORT_NOTES.md). Until it's added, this loop finds
     * zero endpoints, no subdevs get added to the notifier, bound()/
     * complete() never fire, and isp_v4l2_create_instance() is never
     * called -- i.e. the driver loads and sits idle rather than
    ...
    while ( idx < V4L2_SOC_SUBDEV_NUMBER ) {
        ...
        if ( !remote ) {
            LOG( LOG_ERR, "endpoint %d has no remote port parent, skipping", idx );
            continue;
        }
