...
/* 4.9->6.1 port: <linux/pgtable.h> must be included before <asm/uaccess.h>
 * here -- arm64's asm/uaccess.h -> asm/mmu.h now references pgprot_t in a
 * function prototype, and nothing pulled that typedef in first on its own
 * (worked by accident via some other transitive include on 4.9). Without
 * this, arch/arm64/include/asm/mmu.h fails with "unknown type name
 * 'pgprot_t'". */
