...
/* 4.9->6.1 port: <linux/dma-contiguous.h> is gone; dropped (unused in this
 * file -- no dma_alloc_from_contiguous/etc calls here, same situation as
 * isp-v4l2-stream.c). */
...
static int write_to_file(char *buf, int size)
{
	...
	/* 4.9->6.1 port: get_fs()/set_fs(KERNEL_DS)/vfs_write() on a kernel
	 * buffer -> kernel_write(), same rewrite as main_kernel_juno_v4l2.c's
	 * write_to_file(). Also fixed filp_open()'s error check from
	 * "if (!fp)" to IS_ERR(fp) while here -- filp_open() has always
	 * returned ERR_PTR() on failure, never NULL, same pre-existing bug
	 * as the other write_to_file(). */
