...
int write_index_to_file(char *path, char *buf, int index, int size)
{
	...
	/* radxa-zero2pro-camera: set_fs()/KERNEL_DS and vfs_write() on a kernel
	 * buffer are gone. set_fs() was removed in v5.10 (commit 47058bb54b57,
	 * "arm64: remove set_fs implementation"), and vfs_write() rejects kernel
	 * pointers without it. kernel_write() is the supported replacement and
	 * needs no address-space games. */
...
static int write_data_to_buf(char *path, char *buf, int size)
{
	...
	/* See write_index_to_file() above: kernel_read() replaces the
	 * set_fs(KERNEL_DS) + vfs_read() pattern. */
