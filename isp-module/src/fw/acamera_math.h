...
/* 4.9->6.1 port: renamed from array_size() to ACAMERA_ARRAY_SIZE() --
 * <linux/overflow.h> (pulled in transitively via <linux/string.h> et al
 * on 6.1) now defines its own 2-arg array_size(a, b) = size_mul(a, b)
 * used for allocation-size overflow checking. Same name, different
 * arity/semantics; keeping both alive under one name is asking for a
 * confusing bug later, so renamed ours instead of relying on macro
 * redefinition (redefinition also depends on include order, which
 * isn't guaranteed across this many translation units). */
#define ACAMERA_ARRAY_SIZE( a ) \
    ( sizeof( a ) / sizeof( a[0] ) )
