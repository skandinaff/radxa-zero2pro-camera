...
/* 4.9->6.1 port: <asm/uaccess.h> alone only gets you raw_copy_from_user()
 * on this arch/kernel; the checked copy_from_user() wrapper used below
 * (ae_manual_func.c had the identical issue) lives in <linux/uaccess.h>. */
